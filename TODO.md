...
- [x] 实现知识库相关功能（上传、浏览、下载、Star）
- [x] 实现人设卡相关功能（上传、浏览、下载、Star）
- [x] 实现审核功能（admin/moderator）
- [x] 实现审核管理页面（review_tab_content.dart，支持知识库和人设卡审核，包含分页功能）
- [x] 实现消息/信箱功能
- [x] 实现公告功能(与消息/信箱功能同步)
- [x] 实现服务器设置页面（profile_tab_content.dart，支持管理员修改API地址，包含身份验证）
- [x] 实现后端服务地址切换功能（支持动态修改API地址，使用SharedPreferences持久化存储）
- [x] API接口优化和性能提升（2025-11-23）
  - [x] 优化Star功能，使用新的Star状态检查接口（`/api/knowledge/{id}/starred`、`/api/persona/{id}/starred`）
  - [x] 增强用户Star记录接口，支持`includeDetails`参数，减少API调用次数
  - [x] 实现公开列表分页支持（知识库和人设卡）
  - [x] 实现待审核列表分页支持
  - [x] 创建分页响应模型`PaginatedResponse<T>`
- [x] 核心服务层优化（2025-11-23）
  - [x] API服务异步初始化优化，使用Completer确保初始化完成
  - [x] Token动态获取，每次请求时重新读取SharedPreferences
  - [x] 扩展Knowledge和Persona数据模型，添加新字段
  - [x] 管理员功能增强（上传管理、概览、审核页面优化）
  - [x] 消息系统优化（支持分页、已读状态管理）
- [x] 收藏功能增强（状态检查接口、分页/排序、详情包含）
- [x] 知识库详情支持文件级下载/删除，收藏计数实时刷新
- [x] “我的内容”管理页（知识库/人设卡分页筛选排序、删除）
- [x] 公开/用户内容列表支持上传者筛选与排序
- [x] SessionStore 全局401处理与登录错误解析优化
- [x] 人设卡更新接口与知识库文件删除接口对接
